...
int main(int argc, char **argv)
{
    ...
    do {
        ...
        if (op > 2) {
            ...
            if (isValid(text)) {
                if (op==1) {
                    wcout<<L"Encrypted text: "<<cipher.CoderPerestanCipher(cipher, text)<<endl;
                } else {
                    wcout<<L"Decrypted text: "<<cipher.DecoderPerestanCipher(cipher, text)<<endl;
                }
            } else {
